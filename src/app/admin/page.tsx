    ...
    return (
        <div className="space-y-10 p-2">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-2xl font-black tracking-tighter capitalize italic pr-4">Admin <span className="text-primary NOT-italic">Dashboard</span></h1>
                    <p className="text-muted-foreground font-bold text-sm capitalize tracking-widest mt-2 italic opacity-70">See how your store is performing today.</p>
                </div>
                <Badge className="h-12 px-6 rounded-2xl bg-black text-white font-black capitalize tracking-widest text-[10px] italic">
                    ...
                    <Card key={stat.title} className="border-2 shadow-2xl shadow-black/5 rounded-[2rem] overflow-hidden bg-white group hover:border-primary transition-all duration-500">
                        <CardContent className="p-6 md:p-8">
                            <div className="flex justify-between items-start">
                                ...
                                <div className={`flex items-center gap-1 font-black text-[10px] capitalize tracking-tighter ${stat.trend.startsWith('+') ? 'text-emerald-600' : 'text-rose-500'}`}>
                                    {stat.trend.startsWith('+') ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}
                                    {stat.trend}
                                </div>
                            </div>
                            <div className="mt-8">
                                <p className="text-[10px] font-black text-muted-foreground capitalize tracking-[0.2em] italic opacity-60">{stat.title}</p>
                                <h3 className="text-xl font-black mt-2 tracking-tighter italic">
                    ...
                    <CardContent className="h-[400px] pt-8">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={salesData}>
                                ...
                                <Tooltip
                                    contentStyle={{ borderRadius: '1.5rem', border: '2px solid hsl(var(--primary) / 0.1)', boxShadow: '0 20px 40px -10px rgb(0 0 0 / 0.1)', fontWeight: 900, textTransform: 'capitalize', fontSize: '10px', padding: '15px' }}
                    ...
                    <CardContent className="space-y-4 md:space-y-6 pt-6 md:pt-8 px-4 md:px-8 pb-6 md:pb-8">
                        {recentOrders.length === 0 && !loading ? (
                            <div className="h-64 flex flex-col items-center justify-center text-center opacity-30">
                                ...
                                <p className="font-black capitalize tracking-widest text-[10px]">No recent orders found</p>
                            </div>
                        ) : (
                            recentOrders.map((order) => (
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center font-black text-[10px] ring-4 ring-transparent group-hover/signal:ring-primary/10 transition-all capitalize italic">
                                            #{order._id.substring(order._id.length - 4)}
                                        </div>
                                        <div>
                                            <p className="text-sm font-black capitalize italic tracking-tight">{order.shippingAddress?.name || "ANON"}</p>
                                            <div className="flex items-center gap-2 mt-1">
                                                <Badge className="text-[8px] font-black capitalize tracking-tighter h-5 px-2 bg-primary/10 text-primary border-none">{order.orderStatus}</Badge>
                                                <span className="text-[9px] font-black text-muted-foreground capitalize opacity-40 italic">{format(new Date(order.createdAt), "HH:mm")}</span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <Link href="/admin/orders" className="block w-full text-center py-6 text-[10px] font-black capitalize tracking-[0.4em] text-muted-foreground hover:text-primary transition-all border-t-2 border-dashed mt-6 italic group-hover:tracking-[0.5em]">
                            View All Orders →
                        </Link>
                    </CardContent>
                </Card>
